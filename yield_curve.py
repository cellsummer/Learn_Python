...
class YieldCurve:
    def __init__(self, par_rates):
        ...
        self.key_tenor_months = np.concatenate(
            (np.array([0, 3]), np.arange(6, self.last_tenor * 12 + 1, 6)), axis=0
        )
    ...
    def get_key_yield_curve(self):
        ...
        temp_par_rates = np.concatenate(([self.par_rates[0]], self.par_rates), axis=0)
        # temp_tenor_months = list(itertools.chain([0], self.tenor_months))
        temp_tenor_months = np.concatenate(([0], self.tenor_months), axis=0)
    ...
    def get_key_df(self):
        ...
        shape = len(key_par_rates)
        # key_dfs = []
        key_dfs = np.zeros(shape)
        sum_df = 0
        for i, (rate, tenor) in enumerate(zip(key_par_rates, self.key_tenor_months)):
            if tenor > 3:
                key_dfs[i] = (1 - rate / 2 * sum_df) / (1 + rate / 2)
                sum_df = sum(key_dfs[0 : i + 1])

        key_dfs[1] = pow(1 + key_par_rates[0] / 2, -0.5)
        key_dfs[0] = 1
        return key_dfs
    ...
    def get_key_fwd_rate(self):
        ...
        shape = len(key_dfs)
        key_fwd_rates = np.zeros(shape)
        for i, (df, df_6, tenor) in enumerate(
            zip(key_dfs, key_dfs[1:], self.key_tenor_months)
        ):
            if 360 > tenor > 3:
                fwd_rate = pow(df / df_6, 2) - 1
                key_fwd_rates[i] = fwd_rate
                # print(f"{tenor}:{df}:{df_6}:{fwd_rate}")
        # cash rate and 3-month rate
        # print(f"cash rate:{cash_rate}")
        key_fwd_rates[1] = pow(key_dfs[1] / key_dfs[2], 4) - 1
        key_fwd_rates[0] = pow(key_dfs[0] / key_dfs[1], 4) - 1
        # extrapolate
        key_fwd_rates[-1] = key_fwd_rates[-2]
    ...
    def get_full_fwd_rates(self):
        ...
        return full_fwd_rates_interp(np.arange(361))

    def get_full_df(self):
        """calcualte the full discounting factors using the full fwd rates"""
        full_fwd_rates = self.get_full_fwd_rates()
        shape = len(full_fwd_rates)
        single_mth_df = pow(1 + full_fwd_rates[: shape - 1], -1 / 12)
        full_df = np.concatenate(([1], single_mth_df.cumprod()[0:shape]), axis=0)
        return full_df
    ...
    def get_full_spot_rates(self):
        ...
        spot_rates = pow(full_dfs[1:], -12 / self.full_tenor_months[1:]) - 1
        spot_rates = np.concatenate(([0], spot_rates), axis=0)
        return spot_rates
